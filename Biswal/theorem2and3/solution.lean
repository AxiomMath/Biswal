...
theorem D_coeff_mk (K : Type*) [Field K] (m a b : ℕ) :
  ...
  ext n
  simp [D_coeff, PowerSeries.coeff_mk]
...
theorem thm_manifest_coeff_formula (m μ : ℕ) {s : ℕ}
  ...
  exact coeff_prod_mk_eq_sum_antidiag k r (fun ν => D_coeff ℚ m (pairs ν).1 (pairs ν).2)

/-! ## Theorem 3: Part (ii) - Nonnegativity of D_coeff -/
...
private lemma laplace_expansion_two_terms (n : ℕ) (j₀ : Fin (n + 2)) (hj₀ : j₀.val = n) :
  ...
  have hj₀_eq : (Fin.last n).castSucc = j₀ := Fin.ext (by simp [Fin.val_last, Fin.val_castSucc, hj₀])
...
private lemma md_middle_block_upper_entry_zero (n : ℕ) (a j : Fin (n + 1))
  ...
  apply md_stripMatrix_zero_of_col_ge_row_plus_two
  rw [Fin.succAbove_of_castSucc_lt _ _ (by simp [Fin.lt_def, Fin.val_castSucc]; exact x.prop.2),
      Fin.succAbove_of_le_castSucc _ _ (by simp [Fin.le_def, Fin.val_castSucc]; exact y.prop.1)]
...
private lemma md_middle_block_B_lower_triangular (n : ℕ) (a j : Fin (n + 1)) :
  ...
  intro M _ p q hlt
  show stripMatrix (n + 1) (j.succAbove p.val.val) (a.succAbove q.val.val) = 0
...
private lemma md_middle_block_diag_entry (n : ℕ) (a j : Fin (n + 1))
  ...
  simp [stripMatrix,
    Fin.succAbove_of_castSucc_lt j x.val (by simp [Fin.lt_def, Fin.val_castSucc]; exact x.prop.2),
    Fin.succAbove_of_le_castSucc a x.val (by simp [Fin.le_def, Fin.val_castSucc]; exact x.prop.1),
...
private lemma md_rightBlockEquiv_symm_val (n : ℕ) (a j : Fin (n + 1)) (haj : a.val ≤ j.val)
  ...
  simp [md_rightBlockEquiv, Equiv.symm_trans_apply, md_nestedSubtypeEquiv, md_finSubtypeEquiv]
...
private lemma md_right_block_entry_eq (n : ℕ) (a j : Fin (n + 1)) (haj : a.val ≤ j.val)
  ...
  all_goals
    rw [Fin.succAbove_of_le_castSucc _ _ (by
      simp [Fin.le_def, Fin.val_castSucc]
...
private lemma md_rest_block_zero_middle_vs_right (n : ℕ) (a j : Fin (n + 1))
  ...
  show (stripMatrix (n + 1)) (j.succAbove i.val) (a.succAbove j'.val) = 0
  apply md_stripMatrix_zero_of_col_ge_row_plus_two
  have h₁ : (j.succAbove i.val).val = i.val.val := by
    rw [Fin.succAbove_of_castSucc_lt _ _ (by simp [Fin.lt_def, Fin.val_castSucc]; exact hi)]
    simp [Fin.val_castSucc]
...
lemma D_coeff_eq_matrix_inv_coeff (m a b u : ℕ) (hm : 2 ≤ m)
  ...
  rw [stripMatrix_inv_entry m hm ⟨a, by omega⟩ ⟨m - 1 - b, by omega⟩ (by
    simp
    omega)]
  simp only [D_coeff, Nat.sub_sub_self hb]
