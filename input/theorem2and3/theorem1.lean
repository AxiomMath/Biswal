...
private lemma chebyshev_S_eval_pos_at_cos (m : ℕ) (hm : 2 ≤ m) (j : ℕ) (hj : j < m) :
  ...
  have hprod : 0 < Polynomial.eval (2 * Real.cos θ) (Polynomial.Chebyshev.S ℝ ↑j) * Real.sin θ := by
    rw [hcheb]
    exact hsin_jθ
  exact (mul_pos_iff.mp hprod).elim (fun h => h.1) (fun h => absurd hsin_θ (not_lt.mpr (le_of_lt h.2)))
...
lemma remainder_positive_at_roots
  ...
  have h_main : (↑A : PowerSeries ℚ) = (S * D + R : Polynomial ℚ) := by
    ...
    simp [Polynomial.coe_add, Polynomial.coe_mul] at key ⊢
    exact key
...
lemma pos_coeff_transfer_R_to_Q
  ...
  fun r hr => by
    have := h r hr
    simp [PowerSeries.coeff_map] at this
    exact_mod_cast this
...
lemma polyP_natDegree_le (n : ℕ) : (polyP ℝ n).natDegree ≤ n / 2 := by
  ...
  | _ n ih =>
    ...
    | n + 2 =>
      ...
      show (polyP ℝ (n + 1) - Polynomial.X * polyP ℝ n).natDegree ≤ (n + 2) / 2
...
lemma cos_angle_pos (m : ℕ) (hm : 2 ≤ m) (j : Fin (m / 2)) :
  ...
  refine ⟨by linarith [Real.pi_pos, show (0 : ℝ) ≤ (↑(j : ℕ) + 1) * Real.pi / (↑m + 1) from by positivity], ?_⟩
...
lemma candidate_is_root (m : ℕ) (hm : 2 ≤ m) (j : Fin (m / 2)) :
  ...
  have h_sin_zero : Real.sin ((↑m + 1 : ℝ) * ((↑(j : ℕ) + 1) * Real.pi / (↑m + 1))) = 0 := by
    rw [show (↑m + 1 : ℝ) * ((↑(j : ℕ) + 1) * Real.pi / (↑m + 1)) = (↑(j : ℕ) + 1 : ℝ) * Real.pi from by
...
lemma angle_strict_mono (m k_ρ k_x : ℕ) (_hm : 2 ≤ m) (hk_lt : k_ρ < k_x) :
  ...
  nlinarith [show (k_ρ : ℝ) < (k_x : ℝ) from by norm_cast,
             ...
             show (k_ρ : ℝ) * Real.pi / ((↑m : ℝ) + 1) = (k_ρ : ℝ) * (Real.pi / ((↑m : ℝ) + 1)) from by ring,
             show (k_x : ℝ) * Real.pi / ((↑m : ℝ) + 1) = (k_x : ℝ) * (Real.pi / ((↑m : ℝ) + 1)) from by ring]
...
lemma bezout_core (ρ : ℝ) (hρ_pos : 0 < ρ) (s : ℝ) (hs_pos : 0 < s) (Q : Polynomial ℝ) :
  ...
  calc Polynomial.C (1 / s) * ((Polynomial.X - Polynomial.C ρ) * Q + Polynomial.C s) +
    Polynomial.C (ρ / s) * Q * (-Polynomial.C (1 / ρ) * (Polynomial.X - Polynomial.C ρ))
      = Polynomial.C (1 / s) * ((Polynomial.X - Polynomial.C ρ) * Q) + Polynomial.C (1 / s) * Polynomial.C s +
        -(Polynomial.C (ρ / s) * Polynomial.C (1 / ρ)) * ((Polynomial.X - Polynomial.C ρ) * Q) := by ring
    _ = Polynomial.C (1 / s) * ((Polynomial.X - Polynomial.C ρ) * Q) + 1 +
        -(Polynomial.C (1 / s)) * ((Polynomial.X - Polynomial.C ρ) * Q) := by rw [hC_one, hC_combine]
    _ = 1 := by ring
...
lemma root_index_eq_one (m : ℕ) (hm : 2 ≤ m) (ρ : ℝ) (hρ_pos : 0 < ρ)
  ...
  have ⟨j₀, hj₀_lt, hj₀_sin⟩ : ∃ j₀ : ℕ, j₀ < m ∧
    ...
    ⟨(m + 1) / k, candidate_lt_m m k hm hk_ge hk_bound,
     sin_nonpos_of_pi_lt_lt_two_pi _ (angle_gt_pi m k hm hk_ge hk_bound) (angle_lt_two_pi m k hm hk_ge hk_bound)⟩
...
lemma root_comparison (m : ℕ) (hm : 2 ≤ m) (ρ x : ℝ) (hρ_pos : 0 < ρ) (hx_pos : 0 < x)
  ...
  have h_angle_x_le_pi : ↑k_x * Real.pi / (↑m + 1) ≤ Real.pi :=
    le_of_lt (lt_of_lt_of_le (angle_lt_pi_div_two m k_x hm hk_x_bound) (div_le_self (le_of_lt Real.pi_pos) one_le_two))
...
lemma eval_deriv_eq_neg_inv_rho_mul_eval_S (m : ℕ) (ρ : ℝ) (hρ_pos : 0 < ρ) (S : Polynomial ℝ)
  ...
  have h₁ : Polynomial.derivative (1 - Polynomial.C (1/ρ) * Polynomial.X : Polynomial ℝ) = -Polynomial.C (1/ρ) := by
...
lemma polyP_deriv_recurrence (R : Type*) [CommRing R] (n : ℕ) :
  ...
  calc Polynomial.derivative (polyP R (n + 2))
    ...
    _ = Polynomial.derivative (polyP R (n + 1)) -
      ...
      simp [Polynomial.derivative_X, mul_comm]
...
lemma polyP_neg_deriv_eq_convolution (R : Type*) [CommRing R] (n : ℕ) (hn : 2 ≤ n) :
  ...
  have key : ∀ k, 2 ≤ k → -Polynomial.derivative (polyP R k) =
    ...
    | k + 2 =>
      ...
      show -Polynomial.derivative (polyP R (k + 4)) =
...
lemma eval_deriv_polyP_neg (m : ℕ) (hm : 2 ≤ m) (ρ : ℝ) (_hρ_pos : 0 < ρ)
  ...
  have heval : Polynomial.eval ρ (-Polynomial.derivative (polyP ℝ m)) =
    ...
    rw [hconv, Polynomial.eval_finset_sum]
    congr 1
    ext j
...
lemma polyP_factor_at_root (m : ℕ) (hm : 2 ≤ m) (ρ : ℝ) (hρ_pos : 0 < ρ)
  ...
  · have hS_pos := S_eval_pos m hm ρ hρ_pos hρ_root hρ_lower S hS
    have hunit : IsUnit (-Polynomial.C (1 / ρ) : Polynomial ℝ) :=
      (Polynomial.isUnit_C.mpr (isUnit_iff_ne_zero.mpr (div_ne_zero one_ne_zero (ne_of_gt hρ_pos)))).neg
    rw [L_eq_neg_C_mul ρ hρ_pos]
    exact (isCoprime_mul_unit_left_left hunit _ _).mpr (isCoprime_of_eval_pos ρ hρ_pos S hS_pos).symm
...
lemma L_coe_eq_rescale_one_sub_X (ρ : ℝ) :
  ...
  have h1 : (↑L : PowerSeries ℝ) = 1 - PowerSeries.C (1 / ρ) * PowerSeries.X := by
    simp [L, Polynomial.coe_sub, Polynomial.coe_one, Polynomial.coe_mul, Polynomial.coe_C, Polynomial.coe_X]
  have h2 : PowerSeries.rescale (1 / ρ) (1 - PowerSeries.X) = 1 - PowerSeries.C (1 / ρ) * PowerSeries.X := by
    ext n
...
lemma inv_one_sub_pow_eq_invOneSubPow (k : ℕ) :
  ...
  have h := PowerSeries.one_sub_pow_mul_invOneSubPow_val_add_eq_invOneSubPow_val (S := ℝ) (d := 0) (e := k)
...
lemma abs_sum_le_pow_mul_sum_abs (q : Polynomial ℝ) (hd : 1 ≤ q.natDegree) (x : ℝ) (hx : 1 ≤ x) :
  ...
  calc |∑ i ∈ Finset.range q.natDegree, q.coeff i * x ^ i|
    ...
    _ = ∑ i ∈ Finset.range q.natDegree, |q.coeff i| * x ^ i :=
        Finset.sum_congr rfl fun i _ => by rw [abs_mul, abs_of_nonneg (pow_nonneg (le_trans zero_le_one hx) i)]
...
noncomputable def N_scaled (ρ : ℝ) (N_poly : Polynomial ℝ) : Polynomial ℝ :=
  N_poly.comp (Polynomial.C ρ * Polynomial.X)

lemma N_scaled_coe_eq_rescale (ρ : ℝ) (N_poly : Polynomial ℝ) :
    (↑(N_scaled ρ N_poly) : PowerSeries ℝ) =
    (PowerSeries.rescale ρ) (↑N_poly : PowerSeries ℝ) := by
  have h_main : ∀ (n : ℕ), (Polynomial.coeff (N_poly.comp (Polynomial.C ρ * Polynomial.X)) n : ℝ) = (ρ : ℝ) ^ n * Polynomial.coeff N_poly n := by
    intro n
    ...
    | monomial k a =>
      ...
      · simp
  ext n
  simp [N_scaled, PowerSeries.coeff_rescale, h_main]

lemma N_scaled_eval_one (ρ : ℝ) (N_poly : Polynomial ℝ) :
    (N_scaled ρ N_poly).eval 1 = N_poly.eval ρ := by
  simp [N_scaled, Polynomial.eval_comp, Polynomial.eval_mul, Polynomial.eval_C, Polynomial.eval_X]
...
lemma hilbertPoly_pos_leadingCoeff (ρ : ℝ) (_hρ : 0 < ρ) (k : ℕ) (hk : 0 < k)
    ...
    0 < ((N_scaled ρ N_poly).hilbertPoly k).leadingCoeff := by
...
lemma scaled_coeff_is_eventually_poly
    ...
    ∃ (q : Polynomial ℝ) (N₀ : ℕ),
      ...
      ∀ r : ℕ, N₀ < r →
        ...
        q.eval (r : ℝ) := by
  intro L
  refine ⟨(N_scaled ρ N_poly).hilbertPoly k, (N_scaled ρ N_poly).natDegree,
...
lemma inv_one_pow_coeff_bound (k : ℕ) (_hk : 0 < k) (ρ₁ : ℝ) (hρ₁_pos : 0 < ρ₁) :
  ...
  cases r with
  | zero => simp [abs_of_nonneg]
  | succ n =>
    simp_all [abs_of_nonneg, pow_succ]
    positivity
...
lemma coeff_bound_strengthened
  ...
  calc |(PowerSeries.coeff j) f|
    ...
    _ ≤ C * (↑r + 1) ^ D * (q ^ r * q⁻¹ ^ i) := by
        ...
        · exact mul_nonneg (pow_nonneg (le_of_lt hq) _) (pow_nonneg (inv_nonneg.mpr (le_of_lt hq)) _)
...
lemma multiset_prod_inv_bound
  ...
  | cons a s ih =>
    ...
    refine ⟨C', D', hC', ?_⟩
    intro r
    show |(PowerSeries.coeff r) (Multiset.map (fun α =>
      (((↑(1 - Polynomial.C (1/α) * Polynomial.X : Polynomial ℝ) : PowerSeries ℝ) ^ k)⁻¹))
      (a ::ₘ s)).prod| ≤ _
...
lemma prod_X_sub_C_eq_scalar_mul_prod_L (roots : Multiset ℝ) (hroots : ∀ α ∈ roots, (0 : ℝ) < α) :
  ...
  conv_lhs =>
    rw [show roots.map (fun α => Polynomial.X - Polynomial.C α) =
        roots.map (fun α => -Polynomial.C α * (1 - Polynomial.C (1/α) * Polynomial.X : Polynomial ℝ))
...
lemma prod_neg_C_eq_C_prod_neg (roots : Multiset ℝ) :
  ...
  have h2 : (roots.map (fun α => Polynomial.C (-α))).prod = Polynomial.C ((roots.map Neg.neg).prod) := by
...
lemma coe_pow_eq_prod_coe_pow
    (S : Polynomial ℝ)
    (hS_eq : S = (S.roots.map (fun α => (1 - Polynomial.C (1/α) * Polynomial.X : Polynomial ℝ))).prod)
    (k : ℕ) :
    (↑S : PowerSeries ℝ) ^ k =
      (S.roots.map (fun α =>
        ((↑(1 - Polynomial.C (1/α) * Polynomial.X : Polynomial ℝ) : PowerSeries ℝ) ^ k))).prod := by
  conv_lhs => rw [hS_eq]
...
theorem scaled_coeff_poly_degree_ge
    ...
    (_hq_lc : 0 < q.leadingCoeff) (hq_eq : ∀ r : ℕ, N₀ < r →
        ρ ^ r * (PowerSeries.coeff r)
          ((↑N_poly : PowerSeries ℝ) * ((↑(1 - Polynomial.C (1/ρ) * Polynomial.X : Polynomial ℝ) : PowerSeries ℝ) ^ k)⁻¹) =
        q.eval (r : ℝ)) :
    k - 1 ≤ q.natDegree := by
  set Q := (N_scaled ρ N_poly).hilbertPoly k with _
  ...
  have hq_eq_Q : q = Q := by
    apply poly_eq_of_eventually_eq q Q (max N₀ (N_scaled ρ N_poly).natDegree)
    intro r hr
    have hr_N₀ : N₀ < r := lt_of_le_of_lt (le_max_left _ _) hr
    have hr_deg : (N_scaled ρ N_poly).natDegree < r :=
      lt_of_le_of_lt (le_max_right _ _) hr
    rw [← hq_eq r hr_N₀]
    rw [show ρ ^ r * (PowerSeries.coeff r)
        ((↑N_poly : PowerSeries ℝ) * ((↑(1 - Polynomial.C (1/ρ) * Polynomial.X : Polynomial ℝ) : PowerSeries ℝ) ^ k)⁻¹) =
        (PowerSeries.coeff r) ((↑(N_scaled ρ N_poly) : PowerSeries ℝ) * (↑(PowerSeries.invOneSubPow ℝ k) : PowerSeries ℝ)) from by
      rw [← PowerSeries.coeff_rescale, map_mul, ← N_scaled_coe_eq_rescale,
        rescale_inv _ _ (by
          have : PowerSeries.constantCoeff ((↑(1 - Polynomial.C (1/ρ) * Polynomial.X : Polynomial ℝ) : PowerSeries ℝ)) = 1 := by
            simp
  ...
  have hroot : Polynomial.rootMultiplicity 1 (N_scaled ρ N_poly) = 0 := by
